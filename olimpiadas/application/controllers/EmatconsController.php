...
class EmatconsController extends Zend_Controller_Action
{
    ...
    public function versesionescolAction()
    {
        // action body
         //configuraci�n inicial
        $config = Zend_Registry::get('config');
        ...
        $S_RBD=$this->_request->getParam('versesionescolp1');
        //
        if($S_RBD!= "")
               {
                 $sql = "SELECT *  FROM olimpiadas.sesiones t where t.institucion = '$S_RBD' order by t.lista";
                 ...
                 try{
                           ...
                           if (count($rowset) > 0)
                           {
                                   Zend_Layout::getMvcInstance()->assign('lassesiones',$rowset);
                           }
